...
/** 
* @author yupeng
* @version 创建时间：2018年10月18日 下午9:11:14 
* 类说明 
*/
...
public class BinarySearchTree {
     ...
     *             若树为空，则抛出异常
     */
    public static TreeNode minElemNode(TreeNode node) throws Exception {
        if (node == null) {
            throw new Exception("树为空！");
        }
        TreeNode pNode = node;
     ...
     * @throws Exception
     */
    public static TreeNode successor(TreeNode node) throws Exception {
        if (node == null) {
            return null;
        }
     ...
     * @throws Exception
     */
    private static void delete(TreeNode pNode) throws Exception {
        if (pNode == null) {
            return;
        }
 ...
 public static void main(String[] args) {
	 try {
         ...
         int[] keys = new int[] { 2,4,12,45,21,6,111 };
         for (int key : keys) {
             bst.insert(key);
         }
         System.out.println("查找树是否为空？ " + (bst.isEmpty() ? "是" : "否"));
